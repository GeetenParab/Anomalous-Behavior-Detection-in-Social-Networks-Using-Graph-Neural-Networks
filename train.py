...
from GCN_GAT.utils.utils import null_metrics, calc_metrics, is_better
import torch
...
assert mode in ['GAT']
assert dataset_name in ['cresci-2015', 'Twibot-22']
...
def forward_one_epoch(epoch, model, optimizer, loss_fn, train_loader, val_loader):
    ...
    for batch in train_loader:
        ...
        out = model(batch.num_property_embedding,
                    batch.edge_index,
                    batch.edge_type)
...
def train():
    ...
    if mode == 'GAT':
        ...
        model = BotGAT(hidden_dim=hidden_dim,
                       dropout=dropout,
                       num_prop_size=data.num_property_embedding.shape[-1]).to(device)
